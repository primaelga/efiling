<?php
/**
 * patTemplate (modificated version, filename changed to ntemplate.php)
 ...
 * Version 2.4-modif
 * 
 * Modified by: Andronicus Riyono <nick@riyono.com>
 * Changes from the original v2.4:
 * + 2004.03.16 
 *   added feature: template type BitwiseAnd with CheckBitVar
 *    how-to: create a patTemplate:tmpl with name="<name>", type="bitwiseand", and checkbitvar="<var>" 
 *            create subs with integer condition like 1, 2, 4, 8, 16, ...
 *            then, addvar your checkbitvar with some value, 
 *            ex: 5, now you'll get sub 1 and 4 displayed
 *            ex: 6, now you'll get sub 2 and 4 displayed
 *            you can create sub with any integer value, ex sub 5
 *            then, addvar your checkbitvar with some value, 
 *            ex: 5, now you'll get sub 1, 4 and 5 displayed
 *            ex: 15, now you'll get sub 1, 2, 4, 5 and 8 displayed
 * 
 * + changed special patTemplate tag format
 *    from <patTemplate> to <!-- patTemplate -->
 *    reason: better rendering in WYSIWYG HTML editor (rendered as comment or not rendered at all)
 * 
 * + fixed errors (most are PHP Notice Error)
 ...
 */
define("patTEMPLATE_TYPE_BITWISEAND", "BITWISEAND");
/**
 * Template type Condition
 * 
 * @const patTEMPLATE_TYPE_CONDITION
...
class patTemplate {
    ...
    { 
        ...
        // Set template tags
        $this->setType($type);
		// outputs utf-8 with displayParsedTemplate()
    ...
    /**
     * Sets the checkbitvar of a bitwiseand Template
     * 
     * The type of the template has to be bitwiseand
     * 
     * @param string $template name of the template
     * @param string $checkbitvar name of the checkbitvariable
     * @access private 
     * @see setTemplateType
     */
    function setCheckbitVar($template, $checkbitvar)
    {
        $template = strtoupper($template);
        $checkbitvar = strtoupper($checkbitvar);

        $this->checkbitvars[$template] = $checkbitvar;
    } 
	/**
    ...
    function getAttribute($template, $attribute)
    {
        ...
        if (isset($this->attributes[$template][$attribute])) return $this->attributes[$template][$attribute];
    } 
    ...
    { 
        ...
        if (isset($attributes['whitespace']) && $attributes['whitespace'])
            array_push($this->whitespace, strtolower($attributes['whitespace'])); 
        // use whitepspace mode from last opened template
        else if (isset($this->whitespace[(count($this->whitespace)-1)])) {
        ...
        switch ($tagname) {
            ...
            case "tmpl": 
                ...
                if (isset($attributes['src'])) {
                    ...
                    if (isset($attributes['parse']) && $attributes['parse'] == "off") {
                        ...
                        // Has the external file to be parsed (by default)
                    } else {
                ...
                switch ($this->template_types[$this->depth]) {
                    ...
                    case "ODDEVEN":
                        ...
                        break; 
					// Template is a BitwiseAnd Tenplate => it needs a checkbit var
                    case "BITWISEAND": 
                        // none found => there is an error
                        if (!$checkbitvar = $attributes['checkbitvar'])
                            die ("Error in template '" . $fname . "': missing checkbitvar for template in line " . $lineno); 
                        // checkbitvar was found => store it
                        $this->setCheckbitVar($this->template_names[$this->depth], $checkbitvar);
                        break; 	
    ...
    {
        if (isset($this->template_data[$this->depth])) $this->template_data[$this->depth] .= $data;
    } 
    ...
    { 
	    ...
	    $this->addVar($template, 'CLASS', $cls);                */
        } 
    } 
    ...
    function parseStandardTemplate($name, $mode = "w")
    {
        ...
        function parseIterativeTemplate($name, $mode)
        {
             ...
             * @access private 
             * @param string $template name of the template
             * @return array $vars		array containing vars
             */
            function getVars($template)
            {
                $vars = array(); 
                // parse all vars
                if (isset($this->variables[$template]) && is_array($this->variables[$template])) {
            ...
            function displayParsedTemplate($name = "")
            {
                ...
                if ($name)
				    if ($this->utf8output) {
/*				        if(1 != $ajax) {
						echo utf8_encode($this->getParsedTemplate($name));
					} else {*/
						return utf8_encode($this->getParsedTemplate($name));
// 					}
				    } else {
/*					if(1 != $ajax) {
    						echo $this->getParsedTemplate($name); 
					} else {*/
						return $this->getParsedTemplate($name); 
// 					}
				    }
...
/*				if(1 != $ajax) {
					...
					return $templates[$this->templates[$i]];
// 				}
			}
                    } 
                } 
            ...
            function getTemplateContent($name)
            {
                ...
                switch ($this->getAttribute($name, "type")) {
                    case patTEMPLATE_TYPE_ODDEVEN:
                        ...
                        return $this->plain_templates[$name][$sub];
                        break;
                    case patTEMPLATE_TYPE_BITWISEAND:
                        $checkbitval = $this->getVar($name, $this->checkbitvars[$name]); 
                        // check, if conditionvalue is empty
                        if (!isset($checkbitval) || (is_string($checkbitval) && $checkbitval == "") || $checkbitval === false)
                            $checkbitval = 0; 
                        // check if there was a match, otherwise use default
						$match_found = false;
						$bitwiseAndBuffer = "";
                        for($i = 0; $i < $this->cnt_subtemplates[$name]; $i++) {
                            if (((int)$this->subtemplate_conditions[$name][$i] & (int)$checkbitval) == (int)$this->subtemplate_conditions[$name][$i]) {
							    $conditionval = $this->subtemplate_conditions[$name][$i];
                                if (isset($this->plain_templates[$name][$conditionval])) {
								    $match_found = true;
                                    $bitwiseAndBuffer .= $this->plain_templates[$name][$conditionval];
                                }
                            } 
                        } 
                        if (!$match_found) {
						    if (isset($this->plain_templates[$name]['default'])) {
						        return $this->plain_templates[$name]['default'];
						    } else {
							    return "";
							}
						} else {
						    return $bitwiseAndBuffer;
						}
                        break;
            ...
            { 
                ...
                if (isset($val) && !$val && $this->getAttribute($template, "useglobals") == "yes")
            ...
            function dump()
            {
                ...
                for ($i = 0; $i < $this->cnt_templates; $i++) {
                    ...
                    if ($this->cnt_subtemplates[$name] > 0) {
                        ...
                        for ($j = 0; $j < $this->cnt_subtemplates[$name]; $j++) {
                            ...
                            if (is_array($matches[0]) && count($matches[0]) > 0) {
                                ...
                                if (isset($matches[1][$k]) && $matches[1][$k] != "" && !isset($this->variables[$name][$matches[1][$k]]) && !isset($this->globals[$matches[1][$k]]))
                                    $unused[] = $matches[0][$k];
                            } 
                    ...
                    // Display Variables
                    if (isset($this->variables[$name]) && is_array($this->variables[$name]) && count($this->variables[$name]) > 0) {
